...
export function ProductCard({ product }: { product: ShopifyProduct }) {
  ...
  return (
    <article className="group flex h-full flex-col rounded-lg border border-border bg-card p-4 transition hover:-translate-y-1 hover:shadow-card">
      <Link to="/product/$handle" params={{ handle: product.node.handle }} className="block">
        ...
        <h3 className="mt-4 font-extrabold">{product.node.title}</h3>
        <p className="mt-1 line-clamp-2 min-h-10 text-sm text-muted-foreground">
          {product.node.description || "Producto disponible en CondiRico"}
        </p>
      </Link>
